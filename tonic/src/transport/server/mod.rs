...
#[derive(Clone)]
pub struct Server<Ex, L = Identity> {
...
impl<L, S, I, F, IO, IE, ResBody> ServeWithShutdown<L, S, I, F, IO, IE, ResBody> for TokioExec
where
...
{
    type BoxFuture = crate::codegen::BoxFuture<(), super::Error>;
...
impl<L, S, I, F, IO, IE, ResBody> ServeWithShutdown<L, S, I, F, IO, IE, ResBody> for LocalExec
where
    IO: AsyncRead + AsyncWrite + Connected + Unpin + Send + 'static,
    ResBody: http_body::Body<Data = Bytes> + 'static,
    ResBody::Error: Into<crate::Error>,
    L: Layer<S> + 'static,
    L::Service: Service<Request<Body>, Response = Response<ResBody>> + Clone + 'static,
    <L::Service as Service<Request<Body>>>::Future: 'static,
    <L::Service as Service<Request<Body>>>::Error: Into<crate::Error> + Send,
    I: Stream<Item = Result<IO, IE>> + 'static,
    IO: AsyncRead + AsyncWrite + Connected + Unpin + Send + 'static,
    IO::ConnectInfo: Clone + Send + Sync + 'static,
    IE: Into<crate::Error> + 'static,
    F: Future<Output = ()> + 'static,
{
    type BoxFuture = crate::codegen::LocalBoxFuture<(), super::Error>;
