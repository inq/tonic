...
use crate::{MultiThreadExecutor, LocalExecutor};
...
pub struct Grpc<T, Ex = MultiThreadExecutor> {
    codec: T,
    ...
    T: Codec,
{
    ...
    /// Change the executor
    pub fn with_local_executor(self) -> Grpc<T, LocalExecutor> {
        Grpc {
            codec: self.codec,
...
macro_rules! register_executor {
    ($executor: ty, $box_body: path $(, $ext: tt)?) => {
        impl<T> Grpc<T, $executor>
        where
            T: Codec,
         {
            ...
            where
                B: Stream<Item = Result<T::Encode, Status>> + $($ext +)* 'static,
            {
                ...
                http::Response::from_parts(parts, <$box_body>::new(body))
            }
...
register_executor!(LocalExecutor, LocalBoxBody);
register_executor!(MultiThreadExecutor, BoxBody, Send);
